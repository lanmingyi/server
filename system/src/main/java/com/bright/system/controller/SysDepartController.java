...
import com.bright.common.util.ImportExcelUtil;
...
import com.bright.system.model.SysDepartTreeModel;
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                ...
                for (SysDepart sysDepart : listSysDeparts) {
                    ...
                    //update-begin---author:lmy ---date:20220105  for：[JTC-363]部门导入 机构类别没有时导入失败，赋默认值------------
					if(ConvertUtils.isEmpty(sysDepart.getOrgCategory())){
					    sysDepart.setOrgCategory("1");
                    }
                    //update-end---author:lmy ---date:20220105  for：[JTC-363]部门导入 机构类别没有时导入失败，赋默认值------------
					ImportExcelUtil.importDateSaveOne(sysDepart, SysDepartService.class, errorMessageList, num, CommonConstant.SQL_INDEX_UNIQ_DEPART_ORG_CODE);
					num++;
                }
				//清空部门缓存
