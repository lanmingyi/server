...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements SysCategoryService {
	...
	public void addSysCategory(SysCategory sysCategory) {
		String categoryCode = "";
		String categoryPid = SysCategoryService.ROOT_PID_VALUE;
		String parentCode = null;
		if(ConvertUtils.isNotEmpty(sysCategory.getPid())){
			categoryPid = sysCategory.getPid();

			//PID 不是根节点 说明需要设置父节点 hasChild 为1
			if(!SysCategoryService.ROOT_PID_VALUE.equals(categoryPid)){
				SysCategory parent = baseMapper.selectById(categoryPid);
				parentCode = parent.getCode();
				if(parent!=null && !SysCategoryService.HAS_CHILD.equals(parent.getHasChild())){
					parent.setHasChild(SysCategoryService.HAS_CHILD);
	...
	public void updateSysCategory(SysCategory sysCategory) {
		if(ConvertUtils.isEmpty(sysCategory.getPid())){
			sysCategory.setPid(SysCategoryService.ROOT_PID_VALUE);
		}else{
	...
	@Override
	public List<TreeSelectModel> queryListByCode(String pcode) throws BaseException {
		...
		if(ConvertUtils.isNotEmpty(pcode)) {
			...
			if(list==null || list.size() ==0) {
				throw new BaseException("该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new BaseException("该编码【"+pcode+"】存在多个，请核实!");
			}
	...
	public List<String> loadDictItem(String ids, boolean delNotExist) {
		...
		//  新增delNotExist参数，设为false不删除数据库里不存在的key ----
		if (delNotExist) {
