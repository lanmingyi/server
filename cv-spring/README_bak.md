...
BrightComplete 人工智能开发平台（后端）
===============

当前最新版本： 1.0.0（发布日期：2022-11-03） 
...
| 项目名         | 说明                     | 
|-------------|------------------------|
| `cv-spring` | SpringBoot后台源码（支持微服务）        |
| `cv-vue`    |Vue2版前端源码     |
...
```
项目结构
├─cv-spirng（父POM： 项目依赖、modules组织）
│  ├─cv-base-core（共通模块： 工具类、config、权限、查询过滤器、注解等）
│  ├─cv-demo    示例代码
│  ├─cv-system  System系统管理目录
│  │  ├─cv-system-biz    System系统管理权限等功能
│  │  ├─cv-system-start  System单体启动项目(8080）
│  │  ├─cv-system-api    System系统管理模块对外api
│  │  │  ├─cv-system-cloud-api   System模块对外提供的微服务接口
│  │  │  ├─cv-system-local-api   System模块对外提供的单体接口
│  ├─cv-server-cloud           --微服务模块
     ├─cv-cloud-gateway       --微服务网关模块(9999)
     ├─cv-cloud-nacos       --Nacos服务模块(8848)
     ├─cv-system-cloud-start  --System微服务启动项目(7001)
     ├─cv-demo-cloud-start    --Demo微服务启动项目(7002)
     ├─cv-visual
        ├─cv-cloud-monitor       --微服务监控模块 (9111)
        ├─cv-cloud-xxljob        --微服务xxljob定时任务服务端 (9080)
        ├─cv-cloud-sentinel     --sentinel服务端 (9000)
        ├─cv-cloud-test           -- 微服务测试示例（各种例子）
           ├─cv-cloud-test-more         -- 微服务测试示例（feign、熔断降级、xxljob、分布式锁）
           ├─cv-cloud-test-rabbitmq     -- 微服务测试示例（rabbitmq）
           ├─cv-cloud-test-seata          -- 微服务测试示例（seata分布式事务）
           ├─cv-cloud-test-shardingsphere    -- 微服务测试示例（分库分表）
```
...
- 安装依赖（自行百度）
```
安装jdk8、Maven、Redis、Mysql
```

- 导入JAVA项目&启动

```
修改配置文件： cv-system/cv-system-start/src/main/resources/application-dev.yml 中的数据库配置(连接和账号密码)和Redis配置（配置redis的host和port）
```
...
```
右键执行类cv-system/cv-system-start/src/main/java/com/brightcomplete/SystemApplication.java 启动项目
```
![](db/2.png)

Docker启动项目
-----------------------------------
- [Docker启动单体后台](http://doc.brightcomplete.com/2043889)
- [Docker启动微服务后台](http://doc.brightcomplete.com/3043472)
- [Docker启动Vue3前端](http://vue3.brightcomplete.com/3028878)
- [Docker启动Vue2前端](http://doc.brightcomplete.com/3043612)
...
- Vue2版本：`Vue2.6+@vue/cli+AntDesignVue+Viser-vue+Vuex等`  [详细查看](https://github.com/lanmingyi/cv-vue)
...
9、链路跟踪 Skywalking   [参考文档](http://doc.brightcomplete.com/2350293)
