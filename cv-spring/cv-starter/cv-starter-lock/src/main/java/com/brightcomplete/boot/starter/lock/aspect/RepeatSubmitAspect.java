package com.brightcomplete.boot.starter.lock.aspect;
...
import com.brightcomplete.boot.starter.lock.annotation.JRepeat;
import com.brightcomplete.boot.starter.lock.client.RedissonLockClient;
import com.brightcomplete.common.exception.CommonCloudException;
        ...
        if (Objects.nonNull(jRepeat)) {
            ...
            try {
                ...
                if (isLocked) {
                    ...
                    // 未获取到锁
                    throw new CommonCloudException("请勿重复提交");
                }
            } finally {
                // 如果锁还存在，在方法执行完成后，释放锁
                if (isLocked) {
