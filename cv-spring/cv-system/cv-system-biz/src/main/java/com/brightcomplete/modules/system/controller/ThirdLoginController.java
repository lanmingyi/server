...
import com.brightcomplete.common.util.PasswordUtil;
import com.brightcomplete.common.util.RedisUtil;
import com.brightcomplete.common.util.RestUtil;
import com.brightcomplete.common.util.oConvertUtils;
import com.brightcomplete.config.thirdapp.ThirdAppConfig;
import com.brightcomplete.config.thirdapp.ThirdAppTypeItemVo;
import com.brightcomplete.modules.base.service.BaseCommonService;
import com.brightcomplete.modules.system.entity.SysThirdAccount;
import com.brightcomplete.modules.system.entity.SysUser;
import com.brightcomplete.modules.system.model.ThirdLoginModel;
import com.brightcomplete.modules.system.service.ISysThirdAccountService;
...
public class ThirdLoginController {
    ...
    public String loginThird(@PathVariable("source") String source, AuthCallback callback,ModelMap modelMap) {
        ...
        if(response.getCode()==2000) {
        	...
        	//判断有没有这个人
			//update-begin-author:lmy date:20201118 for:修改成查询第三方账户表
        	...
        	// 生成token
			//update-begin-author:lmy date:20201118 for:从第三方登录查询是否存在用户id，不存在绑定手机号
			...
			//update-end-author:lmy date:20201118 for:从第三方登录查询是否存在用户id，不存在绑定手机号
		//update-begin--Author:lmy  Date:20200729 for：接口在签名校验失败时返回失败的标识码 issues#1441--------------------
        }else{
			modelMap.addAttribute("token", "登录失败");
		}
		//update-end--Author:lmy  Date:20200729 for：接口在签名校验失败时返回失败的标识码 issues#1441--------------------
	...
	public Result<String> thirdUserCreate(@RequestBody ThirdLoginModel model) {
		...
		//创建新账号
		//update-begin-author:lmy date:20201118 for:修改成从第三方登录查出来的user_id，在查询用户表尽行token
		...
		if(oConvertUtils.isNotEmpty(user.getSysUserId())){
			...
			// 生成token
			String token = saveToken(sysUser);
			//update-end-author:lmy date:20201118 for:修改成从第三方登录查出来的user_id，在查询用户表尽行token
			res.setResult(token);
			res.setSuccess(true);
		}
		return res;
	...
	public Result<JSONObject> getThirdLoginUser(@PathVariable("token") String token,@PathVariable("thirdType") String thirdType) throws Exception {
		...
		//update-begin-author:lmy date:20201118 for:如果真实姓名和头像不存在就取第三方登录的
		...
		//update-end-author:lmy date:20201118 for:如果真实姓名和头像不存在就取第三方登录的
	...
	public String oauth2LoginCallback(@PathVariable("source") String source, @RequestParam("state") String state, HttpServletResponse response) throws Exception {
		...
		if (ThirdAppConfig.WECHAT_ENTERPRISE.equalsIgnoreCase(source)) {
			...
			// 跟随authCode原样返回。
			builder.append("&state=").append(state);
            //update-begin---author:lmy ---date:20220613  for：[issues/I5BOUF]oauth2 钉钉无法登录------------
            builder.append("&prompt=").append("consent");
            //update-end---author:lmy ---date:20220613  for：[issues/I5BOUF]oauth2 钉钉无法登录--------------
            url = builder.toString();
		} else {
			return "不支持的source";
		}
