...
import com.brightcomplete.common.system.vo.DictQuery;
...
import com.brightcomplete.modules.system.entity.SysDict;
import com.brightcomplete.modules.system.entity.SysDictItem;
import com.brightcomplete.modules.system.model.SysDictTree;
import com.brightcomplete.modules.system.model.TreeSelectModel;
import com.brightcomplete.modules.system.security.DictQueryBlackListHandler;
import com.brightcomplete.modules.system.service.ISysDictItemService;
import com.brightcomplete.modules.system.service.ISysDictService;
import com.brightcomplete.modules.system.vo.SysDictPage;
import com.brightcomplete.poi.excel.ExcelImportCheckUtil;
...
public class SysDictController {
	...
	public Result<List<DictModel>> getDictItems(@PathVariable("dictCode") String dictCode, @RequestParam(value = "sign",required = false) String sign,HttpServletRequest request) {
		...
		// 【安全机制】字典接口、online报表、online图表等接口，加一些安全机制
			...
			try {
				...
				for (int i=0;i< list.size();i++) {
					...
					try {
						...
						if(integer>0){
							successLines++;
                        //update-begin---author:lmy ---date:20220211  for：[JTC-1168]如果字典项值为空，则字典项忽略导入------------
						}else if(integer == -1){
                            errorLines++;
                            errorMessage.add("字典名称：" + po.getDictName() + "，对应字典列表的字典项值不能为空，忽略导入。");
                        }else{
                        //update-end---author:lmy ---date:20220211  for：[JTC-1168]如果字典项值为空，则字典项忽略导入------------
							errorLines++;
							int lineNumber = i + 1;
                            //update-begin---author:lmy ---date:20220209  for：[JTC-1168]字典编号不能为空------------
                            ...
                            //update-end---author:lmy ---date:20220209  for：[JTC-1168]字典编号不能为空------------
                        }
					}  catch (Exception e) {
						errorLines++;
						int lineNumber = i + 1;
