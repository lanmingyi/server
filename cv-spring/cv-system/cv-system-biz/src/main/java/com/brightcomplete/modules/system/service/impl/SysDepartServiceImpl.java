...
import com.brightcomplete.common.constant.FillRuleConstant;
import com.brightcomplete.common.constant.SymbolConstant;
import com.brightcomplete.common.util.FillRuleUtil;
...
import com.brightcomplete.modules.system.service.ISysDepartService;
import com.brightcomplete.modules.system.util.FindsDepartsChildrenUtil;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	public List<SysDepartTreeModel> queryTreeList() {
        ...
        //update-begin---author:lmy ---date:20220307  for：[JTC-119]在部门管理菜单下设置部门负责人 创建用户的时候不需要处理
		//设置用户id,让前台显示
	...
	public void saveDepartData(SysDepart sysDepart, String username) {
		if (sysDepart != null && username != null) {
            ...
            //update-end---author:lmy ---date:20220307  for：[JTC-119]在部门管理菜单下设置部门负责人 创建用户的时候不需要处理
         }
	...
	private String[] generateOrgCode(String parentId) {	
		//update-begin--Author:lmy  Date:20190201 for：组织机构添加数据代码调整
				LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
				...
				strArray[1] = orgType;
				return strArray;
		//update-end--Author:lmy  Date:20190201 for：组织机构添加数据代码调整
	} 
	...
	public List<SysDepartTreeModel> queryTreeListByPid(String parentId,String ids, String primaryKey) {
		...
		lqw.func(square);
        //update-begin---author:lmy ---date:20220527  for：[VUEN-1143]排序不对，vue3和2应该都有问题，应该按照升序排------------
		lqw.orderByAsc(SysDepart::getDepartOrder);
        //update-end---author:lmy ---date:20220527  for：[VUEN-1143]排序不对，vue3和2应该都有问题，应该按照升序排--------------
		List<SysDepart> list = list(lqw);
        //update-begin---author:lmy ---date:20220316  for：[JTC-119]在部门管理菜单下设置部门负责人 创建用户的时候不需要处理
        //设置用户id,让前台显示
        this.setUserIdsByDepList(list);
        //update-end---author:lmy ---date:20220316  for：[JTC-119]在部门管理菜单下设置部门负责人 创建用户的时候不需要处理
    ...
    //update-begin---author:lmy ---date:20200308  for：[JTC-119]在部门管理菜单下设置部门负责人，新增方法添加部门负责人、删除负责部门负责人、查询部门对应的负责人
    /**
     * 通过用户id设置负责部门
    ...
    //update-end---author:lmy ---date:20200308  for：[JTC-119]在部门管理菜单下设置部门负责人，新增方法添加部门负责人、删除负责部门负责人、查询部门对应的负责人
