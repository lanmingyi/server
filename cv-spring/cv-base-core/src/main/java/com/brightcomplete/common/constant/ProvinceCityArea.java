package com.brightcomplete.common.constant;
...
public class ProvinceCityArea {
    ...
    public String getCode(String text){
        ...
        if(areaList!=null && areaList.size()>0){
            for(int i=areaList.size()-1;i>=0;i--){
                //update-begin for: 导入 省市区导入后 导入数据错乱 北京市/市辖区/西城区-->山西省/晋城市/城区
                ...
                //update-end for: 导入 省市区导入后 导入数据错乱 北京市/市辖区/西城区-->山西省/晋城市/城区
            }
        }
        return null;
    }

    // update-begin for: 数据导入错误 省市区组件，文件中为北京市，导入后，导为了山西省
    /**
     * 获取省市区code，精准匹配
    ...
    // update-end for: 数据导入错误 省市区组件，文件中为北京市，导入后，导为了山西省
                    ...
                    for(String cityKey:cityJson.keySet()){
                        ...
                        if(areaJson!=null){
                            for(String areaKey:areaJson.keySet()){
                                ...
                                //update-begin for: online 导入 省市区导入后 导入数据错乱 北京市/市辖区/西城区-->山西省/晋城市/城区
                                area.setAheadText(cityJson.getString(cityKey));
                                //update-end for: online 导入 省市区导入后 导入数据错乱 北京市/市辖区/西城区-->山西省/晋城市/城区
