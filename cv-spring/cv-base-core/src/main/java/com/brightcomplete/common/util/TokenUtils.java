package com.brightcomplete.common.util;
...
import com.brightcomplete.common.exception.Base401Exception;
...
public class TokenUtils {
    ...
    public static boolean verifyToken(String token, CommonAPI commonApi, RedisUtil redisUtil) {
        if (StringUtils.isBlank(token)) {
            throw new Base401Exception("token不能为空!");
        }

        // 解密获得username，用于和数据库进行对比
        String username = JwtUtil.getUsername(token);
        if (username == null) {
            throw new Base401Exception("token非法无效!");
        }
        ...
        if (user == null) {
            throw new Base401Exception("用户不存在!");
        }
        // 判断用户状态
        if (user.getStatus() != 1) {
            throw new Base401Exception("账号已被锁定,请联系管理员!");
        }
        // 校验token是否超时失效 & 或者账号密码是否错误
        if (!jwtTokenRefresh(token, username, user.getPassword(), redisUtil)) {
            throw new Base401Exception(CommonConstant.TOKEN_IS_INVALID_MSG);
        }
        return true;
    }
