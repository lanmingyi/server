...
import com.brightcomplete.common.util.filter.FileTypeFilter;
...
public class CommonUtils {
    ...
    public static String getFileName(String fileName){
        ...
        //update-beign for: /issues/3381 online 在线表单 使用文件组件时，上传文件名中含%，下载异常
        fileName = fileName.replaceAll(FILE_NAME_REGEX, "");
        //update-end for: /issues/3381 online 在线表单 使用文件组件时，上传文件名中含%，下载异常
        return fileName;
    }
    ...
    public static String uploadLocal(MultipartFile mf,String bizPath,String uploadpath){
        try {
            //update-begin for: 过滤上传文件类型
            FileTypeFilter.fileTypeFilter(mf);
            //update-end for: 过滤上传文件类型
        ...
        if("".equals(DB_TYPE)) {
            ...
            try {
                ...
                if(dbType.indexOf(DataBaseConstant.DB_TYPE_MYSQL)>=0) {
                    ...
                    //throw new CVBootException("数据库类型:["+dbType+"]不识别!");
                }
            } catch (Exception e) {
                log.error(e.getMessage(), e);
            }finally {
