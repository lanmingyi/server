package com.brightcomplete.common.system.query;
...
import com.brightcomplete.common.system.util.DataAutorUtils;
import com.brightcomplete.common.system.vo.SysPermissionDataRuleModel;
...
import com.brightcomplete.common.constant.DataBaseConstant;
import com.brightcomplete.common.constant.SymbolConstant;
import com.brightcomplete.common.exception.BaseException;
import com.brightcomplete.common.system.util.JwtUtil;
import com.brightcomplete.common.util.SqlInjectionUtil;
...
public class QueryGenerator {
	...
	private static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap) {
		...
		*/
		...
		// update-begin for：如果字段加注解了@TableField(exist = false),不走DB查询
		...
		// update-end for：如果字段加注解了@TableField(exist = false),不走DB查询
	...
	private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap, Set<String> allFields) {
		...
		if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
			...
			//update-begin for: 获取系统用户列表时，使用SQL注入生效
			//判断column是不是当前实体的
			log.info("当前字段有："+ allFields);
			if (!allColumnExist(column, allFields)) {
				throw new BaseException("请注意，将要排序的列字段不存在：" + column);
			}
			//update-end for: 获取系统用户列表时，使用SQL注入生效

			//SQL注入check
			SqlInjectionUtil.filterContent(column);

			//update-begin for：36 多条件排序无效问题修正
			// 排序规则修改
	...
	private static void doSuperQuery(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap, Map<String,String> fieldColumnMap) {
		if(parameterMap!=null&& parameterMap.containsKey(SUPER_QUERY_PARAMS)){
            ...
            // update-begin for：高级查询的条件要用括号括起来，防止和用户的其他条件冲突
            try {
				...
				// update-begin for: 过滤空条件查询，防止 sql 拼接多余的 and
				...
				// update-end for: 过滤空条件查询，防止 sql 拼接多余的 and
                ...
                queryWrapper.and(andWrapper -> {
                    for (int i = 0; i < filterConditions.size(); i++) {
                        ...
                        if (oConvertUtils.isNotEmpty(rule.getField())
                            ...
                            //update-begin for: 【高级查询】 oracle 日期等于查询报错
							...
							// update-begin for：高级查询没有类型转换，查询参数都是字符串类型
							...
							//update-end for: 【高级查询】 oracle 日期等于查询报错
            ...
            // update-end for：高级查询的条件要用括号括起来，防止和用户的其他条件冲突
		}
	...
	public static QueryRuleEnum convert2Rule(Object value) {
		// 避免空数据
		// update-begin for: 查询条件输入空格导致return null后续判断导致抛出null异常
		if (value == null) {
		...
		// update-end for: 查询条件输入空格导致return null后续判断导致抛出null异常
		QueryRuleEnum rule =null;

		//update-begin for：initQueryWrapper组装sql查询条件错误
		...
		//update-end for：initQueryWrapper组装sql查询条件错误

		// step 3 like
		//update-begin for: 默认带*就走模糊，但是如果只有一个*，那么走等于查询
		if(rule == null && val.equals(STAR)){
			rule = QueryRuleEnum.EQ;
		}
		//update-end for: 默认带*就走模糊，但是如果只有一个*，那么走等于查询
	...
	private static Object replaceValue(QueryRuleEnum rule, Object value) {
		...
		String val = (value + "").toString().trim();
		//update-begin for: 查询条件的值为等号（=）
		if(QueryRuleEnum.EQ.getValue().equals(val)){
			return val;
		}
		//update-end for: 查询条件的值为等号（=）
	...
	private static void addQueryByRule(QueryWrapper<?> queryWrapper,String name,String type,String value,QueryRuleEnum rule) throws ParseException {
		if(oConvertUtils.isNotEmpty(value)) {
			//update-begin for：修复逗号分割情况下没有转换类型，导致类型严格的数据库查询报错
			// 针对数字类型字段，多值查询
			...
			addEasyQuery(queryWrapper, name, rule, temp);
			//update-end for：修复逗号分割情况下没有转换类型，导致类型严格的数据库查询报错
		}
	}
	...
	public static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		...
		case IN:
			...
			//update-begin for:【bug】in 类型多值查询 不适配postgresql
			else if(value.getClass().isArray()) {
				queryWrapper.in(name, (Object[])value);
			}else {
				queryWrapper.in(name, value);
			}
			//update-end for:【bug】in 类型多值查询 不适配postgresql
			break;
		case LIKE:
	...
	*/
	...
	private static String getInConditionValue(Object value,boolean isString) {
		//update-begin for: 查询条件如果输入,导致sql报错
		...
		//update-end for: 查询条件如果输入,导致sql报错
	}
