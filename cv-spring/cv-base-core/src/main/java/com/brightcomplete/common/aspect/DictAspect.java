package com.brightcomplete.common.aspect;
...
import com.brightcomplete.common.aspect.annotation.Dict;
...
public class DictAspect {
     ...
     * 定义切点Pointcut
     */
    @Pointcut("execution(public * com.brightcomplete.modules..*.*Controller.*(..)) || @annotation(com.brightcomplete.common.aspect.annotation.AutoDict)")
     ...
     * 字典注入实现 通过对实体类添加注解@dict 来标识需要的字典内容,字典分为单字典code即可 ，table字典 code table text配合使用与原来brightcomplete的用法相同
    ...
    private Object parseDictText(Object result) {
        if (result instanceof Result) {
            if (((Result) result).getResult() instanceof IPage) {
                ...
                //update-begin for：判断是否含有字典注解,没有注解返回
                ...
                //update-end- for：判断是否含有字典注解,没有注解返回
                for (Object record : records) {
                    String json="{}";
                    try {
                        //update-begin for：DictAspect Jackson序列化报错
                        //解决@JsonFormat注解解析不了的问题详见SysAnnouncement类的@JsonFormat
                         json = objectMapper.writeValueAsString(record);
                        //update-end for：DictAspect Jackson序列化报错
                    } catch (JsonProcessingException e) {
                        log.error("json解析失败"+e.getMessage(),e);
                    }
                    //update-begin for：restcontroller返回json数据后key顺序错乱
                    JSONObject item = JSONObject.parseObject(json, Feature.OrderedField);
                    //update-end for：restcontroller返回json数据后key顺序错乱

                    //update-begin for：解决继承实体字段无法翻译问题
        ...
        for (String dictCode : dataListMap.keySet()) {
            ...
            if (needTranslDataTable.size() > 0) {
                ...
                for (DictModel dict : texts) {
                    ...
                    try {
                        // update-begin for: 字典表翻译注解缓存未更新
                        // 保留5分钟
                        redisTemplate.opsForValue().set(redisKey, dict.getText(), 300, TimeUnit.SECONDS);
                        // update-end for: 字典表翻译注解缓存未更新
                    } catch (Exception e) {
    ...
    private String translateDictValue(String code, String text, String table, String key) {
        ...
        for (String k : keys) {
            ...
            //update-begin for：优化微服务应用下存在表字段需要字典翻译时加载缓慢问题
            if (!StringUtils.isEmpty(table)){
            ...
            //update-end for：优化微服务应用下存在表字段需要字典翻译时加载缓慢问题
