package com.brightcomplete.config.shiro;
...
import com.brightcomplete.common.config.TenantContext;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    public LoginUser checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        //update-begin for:校验用户的tenant_id和前端传过来的是否一致
        String userTenantIds = loginUser.getRelTenantIds();
        if(oConvertUtils.isNotEmpty(userTenantIds)){
            String contextTenantId = TenantContext.getTenant();
            String str ="0";
            if(oConvertUtils.isNotEmpty(contextTenantId) && !str.equals(contextTenantId)){
                //update-begin for: 用户租户信息变更判断漏洞
                String[] arr = userTenantIds.split(",");
                if(!oConvertUtils.isIn(contextTenantId, arr)){
                    throw new AuthenticationException("用户租户信息变更,请重新登陆!");
                }
                //update-end for: 用户租户信息变更判断漏洞
            }
        }
        //update-end for:校验用户的tenant_id和前端传过来的是否一致
        return loginUser;
    }
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        ...
        if (oConvertUtils.isNotEmpty(cacheToken)) {
            ...
            //update-begin for：解决每次请求，都重写redis中 token缓存问题
//			else {
//				// 设置超时时间
//				redisUtil.set(CommonConstant.PREFIX_USER_TOKEN + token, cacheToken);
//				redisUtil.expire(CommonConstant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME / 1000);
//			}
            //update-end for：解决每次请求，都重写redis中 token缓存问题
            return true;
        }
