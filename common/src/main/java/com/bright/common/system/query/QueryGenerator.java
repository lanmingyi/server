package com.bright.common.system.query;

import com.alibaba.fastjson.JSON;
import com.baomidou.mybatisplus.annotation.TableField;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.bright.common.constant.CommonConstant;
import com.bright.common.constant.DataBaseConstant;
import com.bright.common.constant.SymbolConstant;
import com.bright.common.exception.BaseException;
import com.bright.common.system.util.DataAutorUtils;
import com.bright.common.system.util.JwtUtil;
import com.bright.common.system.vo.SysPermissionDataRuleModel;
import com.bright.common.util.CommonUtils;
import com.bright.common.util.DateUtils;
import com.bright.common.util.SqlInjectionUtil;
...
public class QueryGenerator {
	...
	private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap, Set<String> allFields) {
		...
		if (ConvertUtils.isNotEmpty(column) && ConvertUtils.isNotEmpty(order)) {
			...
			if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
				...
				String columnStr = ConvertUtils.camelToUnderline(column);
	...
	public static QueryRuleEnum convert2Rule(Object value) {
		// 避免空数据
		//  查询条件输入空格导致return null后续判断导致抛出null异常
		if (value == null) {
	...
	private static void addQueryByRule(QueryWrapper<?> queryWrapper,String name,String type,String value,QueryRuleEnum rule) throws ParseException {
		if(ConvertUtils.isNotEmpty(value)) {
			// 修复逗号分割情况下没有转换类型，导致类型严格的数据库查询报错
			// 针对数字类型字段，多值查询
	...
	public static String installAuthJdbc(Class<?> clazz) {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			if(ruleMap.containsKey(name)) {
				...
				String filedSql = getSingleSqlByRule(rule, ConvertUtils.camelToUnderline(column), value,isString);
